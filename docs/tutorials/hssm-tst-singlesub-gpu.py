#!/usr/bin/env python
# hssm-tst-singlesub-gpu.py — single-subject HSSM, GPU-enabled to match hssm-tst-gpu.py

import os
# --- GPU settings (same pattern as hssm-tst-gpu.py) ---
...
def load_and_filter(csvfile, subj):
    ...
    # valid_upto = N (no cross-subject padding needed for single subject)
    N = len(sdf)
    sdf["valid_upto"] = N
    return sdf
...
def build_nonhier_model(sdf: pd.DataFrame):
    ...
    # Non-hierarchical: no (1|participant_id) terms
    return hssm.HSSM(
        data=sdf,
        model_config=cfg,
        p_outlier=0,
...
def main():
    ...
    idata = model.sample(
        ...
        random_seed=seed,
        cores=1,  # safe on clusters; avoids forking extra writers
        inference_kwargs={            # match GPU script behavior
    ...
    # If this run produced a single chain, stamp chain coord with chain-id
